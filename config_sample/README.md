...
* **area_templates.yaml**
    - Lists the area templates maps can use to generate areas. **Currently unused.**

* **areas.yaml**
    - Contains the default server map.
    - This is the map that is loaded when the server starts, as well as the map the server falls back to when moderators use `/map` on its own.
